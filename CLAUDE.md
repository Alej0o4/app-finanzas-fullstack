...
- **JWT is still in `localStorage`, not httpOnly cookies** — designed in full (Phase 25 spec §25.5) but deferred to a future Phase 26 spec (see `docs/ROADMAP.md`) given the total absence of CSRF protection today and the blast radius across every authenticated request.
